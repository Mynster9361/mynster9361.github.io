...
## DESCRIPTION
The \`Add-AMElement\` function adds an element to an Adaptive Card.
The element can be added directly
to the card body or to a specific container within the card.
This function modifies the card in-place,
...
If the card body does not exist, it will be created automatically.
However, containers must already
...
### -ContainerId
(Optional) The ID of a container to add the element to.
...
### -ColumnId
(Optional) The ID of a column within a container to add the element to.
Only applicable
