...
with the proper format for Microsoft Graph API.
The card is embedded as a JSON payload in a \`\<script\>\` tag
...
This should be a hashtable created using \`New-AMCard\` and
...
### -ToRecipients
(Optional) An array of email addresses to send the email to.
